...
`C2_Containers` — логическая декомпозиция. В текущем Compose механизм правил, retrieval и ERP adapter исполняются внутри API; ingestion запускается командой. Поэтому блоки C2 и `Deployment_MVP` не соответствуют Docker-контейнерам один к одному. Один блок inference объединяет три model services. Langfuse/Prometheus/Grafana входят в observability deployment profile. Сквозная OpenTelemetry/LLM instrumentation приложения, application metrics и готовые project-specific dashboards остаются target; фактический trace процесса хранится в собственном audit store. Docling также относится к target.
