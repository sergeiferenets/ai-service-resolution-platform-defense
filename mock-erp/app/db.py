"""Подключение к PostgreSQL, создание базы мока и миграции."""
...
def ensure_database(s: Settings) -> None:
    """База мока — отдельная от базы платформы: мок изображает внешнюю систему."""
    ...
    with psycopg.connect(conninfo(s)) as conn:
        ...
        for f in sorted(MIGRATIONS.glob("*.sql")):
            if f.stem in applied:
                continue
            # Без параметров psycopg отправляет запрос простым протоколом,
            # поэтому файл миграции может содержать несколько команд.
